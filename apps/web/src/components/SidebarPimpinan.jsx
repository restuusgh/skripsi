...
import { LayoutDashboard, TrendingUp, FileBarChart2, Sparkles, Activity, X, Package, ChevronRight, LogOut, User } from "lucide-react";

// ── Nav items ─────────────────────────────────────────────────────────────────
export const NAV_ITEMS = [
  { path: "/pimpinan/dashboard",   label: "Dashboard",          icon: LayoutDashboard, desc: "Ringkasan eksekutif"      },
  { path: "/pimpinan/grafik-tren", label: "Grafik Tren",        icon: TrendingUp,      desc: "Tren distribusi & stok"  },
  { path: "/pimpinan/laporan",     label: "Laporan",            icon: FileBarChart2,   desc: "Rekap & cetak laporan"   },
  { path: "/pimpinan/prediksi-ai", label: "Hasil Prediksi AI",  icon: Sparkles,        desc: "Output neural network"   },
  { path: "/pimpinan/monitoring",  label: "Monitoring Aktivitas",icon: Activity,       desc: "Log aktivitas sistem"    },
];
...
export const USER = {
  nama:   "H. Ahmad Fauzi",
  email:  "direktur@sawit.id",
  avatar: "https://i.pravatar.cc/100?img=68",
};

// ── Komponen ──────────────────────────────────────────────────────────────────
const SidebarPimpinan = ({ onClose }) => {
  ...
  return (
    <div className="flex flex-col h-full" style={{ background: "#0c1325" }}>
        ...
        <div className="flex flex-col gap-1">
          {NAV_ITEMS.map(({ path, label, icon: Icon, desc }) => (
            <NavLink key={path} to={path} onClick={onClose}
              ...
              onMouseEnter={(e) => { if (!e.currentTarget.classList.contains("active")) e.currentTarget.style.background = ACCENT.bgHover; }}
              onMouseLeave={(e) => { if (!e.currentTarget.classList.contains("active")) e.currentTarget.style.background = "transparent"; }}
            >
              {({ isActive }) => (
                <>
                  <div className="w-8 h-8 rounded-lg flex items-center justify-center shrink-0 transition-all"
                    style={{ background: isActive ? ACCENT.bgLight : "#1a2440", color: isActive ? ACCENT.text : "#5b6c87" }}>
                    <Icon size={16} />
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold leading-none truncate"
                      style={{ color: isActive ? ACCENT.text : "#cbd5e1" }}>{label}</p>
                    <p className="text-[11px] mt-0.5 truncate" style={{ color: "#3a4863" }}>{desc}</p>
                  </div>
                  {isActive && <ChevronRight size={14} style={{ color: ACCENT.text, flexShrink: 0 }} />}
                </>
              )}
            </NavLink>
          ))}
      ...
      <div className="px-3 pb-4 pt-3 border-t flex flex-col gap-1" style={{ borderColor: "#1c2740" }}>
        <button onClick={() => navigate("/pimpinan/profil")}
