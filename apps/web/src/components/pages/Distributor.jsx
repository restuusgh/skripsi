...
import {
  Building2, Plus, Search, Download, Printer, RefreshCw, Eye, Pencil,
  Trash2, X, MapPin, User, Phone, Mail, Package, CheckCircle2,
  XCircle, Clock, Loader2, AlertTriangle, Truck, Globe,
} from "lucide-react";
...
const DUMMY_DISTRIBUTOR = [
  {
    id: 1, nama: "PT Sinar Sawit", kota: "Medan", pic: "Andi Saputra",
    telepon: "08123456789", email: "admin@sinarsawit.com",
    alamat: "Jl. Gatot Subroto No.123", distribusi: 124,
    status: "aktif", produk: ["CPO", "Minyak Goreng"],
  },
  {
    id: 2, nama: "PT Agro Palma", kota: "Pekanbaru", pic: "Budi Hartono",
    telepon: "081398765432", email: "info@agropalma.com",
    alamat: "Jl. Sudirman No.45", distribusi: 83,
    status: "aktif", produk: ["CPO"],
  },
  {
    id: 3, nama: "PT Nusantara Oil", kota: "Dumai", pic: "Rina Wulandari",
    telepon: "081234567890", email: "cs@nusantaraoil.com",
    alamat: "Jl. Pelabuhan No.7", distribusi: 42,
    status: "nonaktif", produk: ["Minyak Goreng"],
  },
];
...
const FORM_KOSONG = {
  nama: "", alamat: "", kota: "", pic: "", telepon: "", email: "", status: "aktif",
};
        ...
        <div className="p-6">
          ...
          <div className="flex flex-col gap-3 mb-5 text-sm">
            <div className="flex items-center gap-2.5 text-slate-300">
              <MapPin size={14} className="text-slate-400 flex-shrink-0" /> {distributor.kota}
            </div>
            <div className="flex items-center gap-2.5 text-slate-300">
              <User size={14} className="text-slate-400 flex-shrink-0" /> {distributor.pic}
            </div>
            <div className="flex items-center gap-2.5 text-slate-300">
              <Phone size={14} className="text-slate-400 flex-shrink-0" /> {distributor.telepon}
            </div>
            <div className="flex items-center gap-2.5 text-slate-300">
              <Mail size={14} className="text-slate-400 flex-shrink-0" /> {distributor.email}
            </div>
            {distributor.alamat && (
              <div className="text-xs text-slate-400 mt-1 pl-6">{distributor.alamat}</div>
            )}
          </div>
          ...
          <div className="border-t border-slate-700 pt-4 mb-4">
            ...
            <div className="flex flex-wrap gap-2">
              {(distributor.produk || []).map((p) => (
                <span key={p} className="bg-slate-900 border border-slate-700 text-slate-300 text-xs px-2.5 py-1 rounded-lg">
                  {p}
                </span>
              ))}
            </div>
        ...
        >
          <div className="flex justify-between items-center mb-5">
            <h3 className="text-base font-extrabold text-slate-100">
              {mode === "edit" ? "Edit Distributor" : "Tambah Distributor"}
            </h3>
            <button onClick={onClose} className="p-1.5 rounded-lg hover:bg-slate-700 transition-colors">
              <X size={18} className="text-slate-400" />
            </button>
          ...
          <div className="flex flex-col gap-3.5">
            <div>
              <label className="text-xs font-semibold text-slate-300 mb-1.5 block">Nama Distributor</label>
              <input
                className="w-full px-3 py-2.5 rounded-lg border border-slate-700 bg-slate-900 text-slate-100 text-sm outline-none"
                placeholder="PT Sinar Sawit"
                value={form.nama}
            ...
            <div>
              <label className="text-xs font-semibold text-slate-300 mb-1.5 block">Alamat</label>
              <input
                className="w-full px-3 py-2.5 rounded-lg border border-slate-700 bg-slate-900 text-slate-100 text-sm outline-none"
                placeholder="Jl. Gatot Subroto No.123"
                value={form.alamat}
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="text-xs font-semibold text-slate-300 mb-1.5 block">Kota</label>
                <input
                  className="w-full px-3 py-2.5 rounded-lg border border-slate-700 bg-slate-900 text-slate-100 text-sm outline-none"
                  placeholder="Medan"
                  value={form.kota}
              ...
              <div>
                <label className="text-xs font-semibold text-slate-300 mb-1.5 block">PIC</label>
                <input
                  className="w-full px-3 py-2.5 rounded-lg border border-slate-700 bg-slate-900 text-slate-100 text-sm outline-none"
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="text-xs font-semibold text-slate-300 mb-1.5 block">Telepon</label>
                <input
                  className="w-full px-3 py-2.5 rounded-lg border border-slate-700 bg-slate-900 text-slate-100 text-sm outline-none"
            ...
            <div>
              ...
              <input
                className="w-full px-3 py-2.5 rounded-lg border border-slate-700 bg-slate-900 text-slate-100 text-sm outline-none"
                placeholder="admin@sinarsawit.com"
                value={form.email}
...
export default function Distributor() {
  ...
  const kotaOptions = useMemo(() => {
    const unik = new Set(list.map((d) => d.kota));
    return ["semua", ...unik];
  }, [list]);
  ...
  const filtered = useMemo(() => {
    return list.filter((d) => {
      ...
      const matchKota = filterKota === "semua" || d.kota === filterKota;
      const matchStatus = filterStatus === "semua" || d.status === filterStatus;
      return matchSearch && matchKota && matchStatus;
    });
  }, [list, search, filterKota, filterStatus]);

  const stats = useMemo(() => {
    const aktif = list.filter((d) => d.status === "aktif").length;
    const nonaktif = list.filter((d) => d.status === "nonaktif").length;
    const kota = new Set(list.map((d) => d.kota)).size;
    const totalDistribusi = list.reduce((sum, d) => sum + (d.distribusi || 0), 0);
    return { total: list.length, aktif, nonaktif, kota, totalDistribusi };
  }, [list]);

  // --- Handler CRUD (ganti body fetch dengan endpoint asli) ---
  const handleSubmitForm = async (form) => {
    ...
    try {
      if (modalForm.mode === "edit") {
        ...
        // const json = await res.json();
        const newItem = { ...form, id: Date.now(), distribusi: 0, produk: [] };
  ...
  const handleExportExcel = () => {
    // Implementasi export bisa pakai lib seperti xlsx / sheetjs
    alert("Export Excel — hubungkan dengan library export sesuai kebutuhan.");
  };
    ...
    <div className="bg-transparent min-h-screen">
      <div className="px-6 lg:px-8 py-8 max-w-[1300px] mx-auto">
          ...
          <div className="flex items-center gap-3.5">
            ...
            <div>
              ...
              <p className="text-slate-400 mt-0.5 text-sm">
                Kelola seluruh distributor dan tujuan distribusi perusahaan
              </p>
            </div>
          </div>

          <button
            onClick={() => setModalForm({ mode: "tambah", data: FORM_KOSONG })}
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-6">
          <StatCard label="Total Distributor" value={stats.total}
            Icon={Building2} color={{ bg: "bg-green-500/10", text: "text-green-500" }} />
          <StatCard label="Aktif" value={stats.aktif}
            Icon={CheckCircle2} color={{ bg: "bg-green-500/10", text: "text-green-500" }} />
          <StatCard label="Nonaktif" value={stats.nonaktif}
            Icon={XCircle} color={{ bg: "bg-red-500/10", text: "text-red-500" }} />
          <StatCard label="Total Kota" value={stats.kota}
            Icon={Globe} color={{ bg: "bg-blue-500/10", text: "text-blue-500" }} />
          <StatCard label="Total Distribusi" value={fmt(stats.totalDistribusi)}
            Icon={Truck} color={{ bg: "bg-yellow-500/10", text: "text-yellow-500" }} />
        </div>
        ...
        <div className="bg-slate-800 border border-slate-700 rounded-2xl p-4 mb-6 shadow-lg flex flex-wrap gap-3 items-center">
          <div className="relative flex-1 min-w-[220px]">
            ...
            <input
              type="text"
              placeholder="Cari distributor atau PIC..."
          ...
          <select
            className="px-3 py-2.5 rounded-lg border border-slate-700 bg-slate-900 text-slate-100 text-sm outline-none min-w-[140px]"
            value={filterKota}
            onChange={(e) => setFilterKota(e.target.value)}
          >
            {kotaOptions.map((k) => (
              <option key={k} value={k}>{k === "semua" ? "Semua Kota" : k}</option>
            ))}
          </select>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="text-left text-slate-400 text-xs uppercase tracking-wide">
                  ...
                  <th className="py-2.5 px-2 font-semibold">Kota</th>
                  <th className="py-2.5 px-2 font-semibold">PIC</th>
                  <th className="py-2.5 px-2 font-semibold">Telepon</th>
                  <th className="py-2.5 px-2 font-semibold">Distribusi</th>
                    ...
                    >
                      ...
                      <td className="py-3 px-2 text-right">
                        <div className="flex items-center gap-1 justify-end">
                          ...
                          <button
                            onClick={() => setModalForm({ mode: "edit", data: d })}
                            className="p-1.5 rounded-lg hover:bg-slate-700 transition-colors"
                            aria-label="Edit"
                          >
